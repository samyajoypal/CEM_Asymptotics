...
\title{Inference for Hard Classification Likelihoods:\
...
\begin{abstract}
Hard classification replaces posterior membership probabilities by
...
their explicit target.  For regular decision hypersurfaces, we show that the
...
the distinct instability caused by optimization-basin selection.  The method
therefore supplies feasible local inference together with diagnostics that
identify when a local Gaussian summary is inadequate.
\end{abstract}

\noindent\textbf{Keywords:} classification likelihood; decision boundary;
finite mixture; M-estimation; nonsmooth inference; sandwich covariance.
...
\input{sections/feasible_inference}
...
analysis show that the correction can restore local calibration without the
cost of a large bootstrap.  Under clear separation the correction contracts,
as predicted by the surface formula.

There are three qualifications.  First, the method estimates a local
classification-likelihood covariance.  It does not make the classification
target equal to the ordinary mixture parameter.  Second, regular active faces,
...
slow its convergence.  Third, global family assignment and optimization-basin
...
\url{https://github.com/samyajoypal/CEM_Asymptotics}. A versioned archival DOI
will be minted for the submitted release. An anonymized reproduction archive
will accompany the initial submission.
