...
The proof is assembled rigorously in Supplementary Lemmas 1.1 and 1.2: the first obtains the
surface term by smoothing the positive part and applying the coarea formula,
and the second shows why inactive equality sets and multiple-tie junctions do
not contribute.

\subsection{A one-dimensional illustration of the boundary term}
\label{sec:one-dimensional-illustration}
The general surface formula becomes especially transparent in one dimension.
Consider two components and suppose their contrast
$g(x;\boldsymbol{\theta})=q_1(x;\boldsymbol{\theta})-
q_2(x;\boldsymbol{\theta})$ has one active regular boundary point $x_0$ at
$\boldsymbol{\theta}^*$.  Equation~\eqref{eq:boundary-curvature} reduces to
\begin{equation}
...
region, explaining why fixed-label inference can be adequate for strongly
separated groups. The following remarks place this example within familiar
hard-clustering geometry and identify the limiting case in which the boundary
term vanishes.
